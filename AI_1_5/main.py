...
from context_strategies import ContextManager, SlidingWindowStrategy, StickyFactsStrategy, BranchingStrategy
...
class Agent:
    def __init__(self, api_config, strategy: str = "sliding", window_size: int = 3):
        self.api_config = api_config
        self.context = ContextManager(strategy_name=strategy, window_size=window_size)
        self._headers = {
    ...
    def count_history_tokens(self):
        return count_messages_tokens(self.get_messages(), self.encoder)
    ...
    def receive_response(self, response):
        if response and 'choices' in response:
            ...
            self.context.add_assistant_message(assistant_reply)
    ...
    def set_strategy(self, strategy: str, **kwargs) -> bool:
        return self.context.set_strategy(strategy, **kwargs)
    
    def get_strategy_info(self) -> dict:
        return self.context.get_context_info()
    
    def switch_branch(self, branch_name: str) -> bool:
        return self.context.switch_branch(branch_name)
    
    def create_branch(self, branch_name: str, from_checkpoint: str | None = None) -> bool:
        return self.context.create_branch(branch_name, from_checkpoint)
    
    def create_checkpoint(self, name: str | None = None) -> dict:
        return self.context.create_checkpoint(name)
...
def main():
    ...
    print("-" * 40)
    print("Strategy commands:")
    print("  /strategy <name>   - Switch strategy: sliding, sticky, branching")
    print("  /info              - Show current strategy info")
    print("  /branch <name>     - Create/switch to branch (branching mode)")
    print("  /checkpoint <name> - Create checkpoint (branching mode)")
    print("-" * 40)
    ...
    while True:
        ...
        if user_input.startswith('/strategy '):
            strategy_name = user_input.split()[1]
            if agent.set_strategy(strategy_name):
                info = agent.get_strategy_info()
                print(f"Strategy switched to: {info['strategy_name']}")
                print(f"  {info}")
            else:
                print(f"Unknown strategy: {strategy_name}. Available: sliding, sticky, branching")
            continue
        
        if user_input == '/info':
            info = agent.get_strategy_info()
            print(f"Current strategy: {info.get('strategy_name', 'unknown')}")
        ...
        if user_input.startswith('/branch '):
            branch_name = user_input.split()[1]
            if agent.context.strategy_name != 'branching':
                print("Switch to branching strategy first: /strategy branching")
                continue
            if agent.switch_branch(branch_name):
                print(f"Switched to branch: {branch_name}")
            else:
                if agent.create_branch(branch_name):
        ...
        if user_input.startswith('/checkpoint '):
            ...
            print(f"Created checkpoint: {result}")
            continue
